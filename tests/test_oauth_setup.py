...
@pytest.mark.asyncio
async def test_connect_rejects_same_value_saved_in_both_oauth_fields(ctx):
    ctx.secrets = MockSecretStore({
        "google_client_id": "duplicated-value",
        "google_client_secret": "duplicated-value",
    })

    result = await handlers.connect_google_drive(ctx, NoParams())

    assert result.status == "error"
    assert result.error_code == "GOOGLE_OAUTH_CREDENTIALS_DUPLICATED"
    assert "same value" in result.error
